...
char* get_IP_or_DATA(char* DATA_from_client, int quotation_marks_set)
{
	char* a = (char*)DATA_from_client + '\0';
	int temp = 0;
	int i, flag=0;
...
void T_run_RS(Socket* s) {
	while (1) {
		...
		if (client_re.size() > 1) {
			...
			//Try to create a new client then using server to send the message,but It doesn't work
			//Check again 
//			SOCKET newclient_tos;
//			struct sockaddr_in Newclient_tosend;
//
//			if ((newclient_tos = socket(AF_INET, SOCK_STREAM, 0)) == INVALID_SOCKET)
//			{
//				printf("Could not create socket : % d", WSAGetLastError());
//			}
//			Newclient_tosend.sin_addr.S_un.S_addr = inet_addr("192.168.1.3");//Client's IP
//			Newclient_tosend.sin_family = AF_INET;
//			Newclient_tosend.sin_port = htons(3333);
//			if (connect(newclient_tos, (struct sockaddr*) & Newclient_tosend, sizeof(Newclient_tosend)), 0)
//			{
//				puts("connect error");
////				return 1;
//			}
			// send to newclient...
//			send(newclient_tos, buf, strlen(buf), 0);
			send(client_re[1], buf, sizeof(buf), NULL);
			
			//		else closesocket(client_connect[i]);
//			closesocket(newclient_tos);
			ZeroMemory(buf, 4096);
		}
